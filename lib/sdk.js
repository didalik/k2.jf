...
function pipe2child (context, indata) { // {{{1
  //console.log('pipe2child context', context, 'indata', indata)
  console.log('pipe2child indata', indata)

  if (!context.job) {
    let jobmatch = context.opts.aud
    ...
    context.job.
      on('error', err => console.error(`pipe2child  E R R O R  ${err}`))
    context.job.stderr.
      on('data', data => console.log(`${jobmatch} stderr`, data.toString()))
    context.job.stdout.
      on('data', data => new JWT(data.toString()).
        setIssuer(context.attachment.iss, context.attachment.sk).sign().
        then(t => context.ws.send(t)))
    context.job.
      on('close', code => new JWT(`${jobmatch} EXIT CODE ${code}`).
        setIssuer(context.attachment.iss, context.attachment.sk).sign().
        then(t => {

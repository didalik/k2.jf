...
function connection (ws2use) { // {{{1
  ...
  console.log('connection browser', browser)
  ...
  let socket = new Proxy(open, {
    get: (_, key) => ({
      ...
      send: (message, recipient) => (
        console.log('connection socket send message', message, 'recipient', recipient, 'ws?.readyState == 1', ws?.readyState == 1),
...
export { // {{{1
  Jobs, Offers, audience, generate_keypair,
}
